...
OPTIONS
   ...
   Option Details
       ...
       -TAG<=DATFILE or -TAG<=FMT
	    ...
	    entries, to conditionally delete/replace tags, or to shift
	    date/time values.
       ...
       -tagsFromFile SRCFILE or FMT
	    ...
	    Copied tags may also be added or deleted from a list, or
	    conditionally deleted/replaced with arguments of the form
	    "'-SRCTAG+<DSTTAG'" or "'-SRCTAG-<DSTTAG'" (but see Note 5 below).
       ...
       -csv[[+]=CSVFILE]
	    ...
	    options to the command after -csv=CSVFILE, or excluded with --TAG,
	    with exclusions taking priority.  Group names and wildcards in tag
	    names are allowed, but the group name must match exactly the group
	    in the CSV file (eg. a "XMP-dc:Subject" column in the CSV file
	    won't match "-XMP:Subject" on the command line.  Also, tags from
	    the CSV may be redirected and written to different tags in the
	    ...
	    existing lists in the file.  This affects only list-type tags.
	    Also applies to the -j option.
       ...
       -j[[+]=JSONFILE] (-json)
	    ...
	    in tag names are allowed, but the group name must match exactly
	    the group in the JSON file (eg. a "XMP-dc:Subject" entry in the
	    JSON file won't match "-XMP:Subject" on the command line.  Also,
	    tags from JSON may be redirected and written to different tags in
	    the target file with support for advanced-formatting expressions
	    using the same syntax as the -tagsFromFile feature.  If no tags
	    are specified, then all tags except FileName, Directory and
	    excluded tags are imported (in the same order as the database
	    entries).
